...
function [E, psi] = bands(k, R_gen, R, tau, r_atom, E_cut, max_G)

    % empirical pseudopotentials [2]
    b_C = [1.781 1.424 0.354 0.938]; % empirical parameters for carbon
    U_C = @(q_in) b_C(1)*(b_C(3)*q_in.^2-b_C(2))./(exp(b_C(3)*q_in.^2-b_C(4))+1); % pseudopotential function for carbon
    b_H = [-0.397 0.0275 0.1745 -0.0531 0.0811 -1.086 2.71 -2.86]; % empirical parameters for hydrogen
    U_H = @(q_in) (q_in<=2).*(b_H(1) + b_H(2)*q_in + b_H(3)*q_in.^2 + b_H(4)*q_in.^3) + (q_in>2).*(b_H(5)./(q_in+(q_in==0)) + b_H(6)./(q_in.^2+(q_in==0)) + b_H(7)./(q_in.^3+(q_in==0)) + b_H(8)./(q_in.^4+(q_in==0))); % pseudopotential function for hydrogen

    n_k = size(k,1); % number of k points
    n_atoms = size(tau,2); % number of atoms within supercell
    V_atom = (4*pi/3)*r_atom.^3; % atomic volumes
    ...
    tic
    parfor i_atom = 1:n_atoms % carbon
        if r_atom(i_atom) == 1 % hydrogen
            U = U + (V_atom(i_atom)/V_cell)*exp(1i*(GmG2)*kron(eye(n_G),tau(:,i_atom))).*U_H(sqrt(abs(GmG2).^2*kron(eye(n_G),ones(3,1))));
        else % carbon
            U = U + (V_atom(i_atom)/V_cell)*exp(1i*(GmG2)*kron(eye(n_G),tau(:,i_atom))).*U_C(sqrt(abs(GmG2).^2*kron(eye(n_G),ones(3,1))));
        end
    end
    toc
    E = zeros(n_k,n_G);
    ...
    for i_psi = 1:n_psi
        for i_k = 1:n_k
            for i_R = 1:n_R
                ...
                k_i = k;
                R_i = R;
                psi(i_R,i_k,i_psi) = (1/V_cell)*sum(u_i{i_k}(:,i_psi).*exp(1i*dot(k_i(i_k,:).*ones(size(G))+G, R_i(i_R,:).*ones(size(G)), 2)), 1);
            end
        end
    end
    toc
